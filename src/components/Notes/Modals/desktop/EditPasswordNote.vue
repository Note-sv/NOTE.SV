  ...
  >
    <q-header :class="txHash ? 'bg-secondary' : 'bg-ino'">
      <q-toolbar>
        <q-toolbar-title>
          {{
            txHash ? $t('EditPassword') : $t('AddPassword')
          }}
        </q-toolbar-title>
        <q-btn
          flat
...
export default {
  ...
  methods: {
    ...
    promptToDelete (id) {
      ...
      this.$q
        ...
        .onOk(() => {
          console.log('deleteNote:', { id: this.id, draft: this.noteToSubmit.draft })
  ...
  async mounted () {
    console.log('edit mounted', this.id)
