    ...
    <q-header class="bg-primary">
      <q-toolbar class="mobile-header">
        <q-btn
          ...
          @click="$router.back()"
        />
        <q-toolbar-title>{{ $t('charge') }}</q-toolbar-title>
      </q-toolbar>
    </q-header>
    <q-page-container>
      <q-page
          ...
          <q-card>
            ...
            >
              <q-tab
                ...
                :label="$t('Wallet')"
                no-caps
                v-show="systemSettings.chargeServices.wallet.mobile"
              />
              <q-tab
                name="chainbow"
                :label="$t('ChainBow')"
              ...
              <q-tab
                ...
                no-caps
                :label="$t('buyBSV')"
                v-show="systemSettings.chargeServices.buybsv.mobile"
              />
              <q-tab
                name="dotwallet"
                no-caps
                :label="$t('DotWallet')"
                v-show="systemSettings.chargeServices.dotwallet.mobile"
              />
              <q-tab
                name="moneybutton"
                :label="$t('MoneyButton')"
                no-caps
                v-show="systemSettings.chargeServices.moneybutton.mobile"
              />
            </q-tabs>
            ...
            >
              <q-tab-panel name="buybsv">
                ...
                <div class="row no-wrap items-center ">
                  ...
                  />
                  <q-space />
                  <div>{{ '1BSV' }}={{ Number(1e8).toLocaleString() + $t('Satoshi') }}</div>
                </div>
              </q-tab-panel>
...
export default {
  ...
  methods: {
    ...
    async chargeViaDotWallet () {
      ...
      try {
        ...
        if (order.data) {
          ...
          console.log(Platform.is, window.open, url, `${url}`)
          openURL(`${url}`)
          this.$q.loading.hide()
          return
        }
    ...
    chargeViaRelayX () {
      const redirectURI = encodeURIComponent(
        `notesv://address/${this.walletAddress}?from=maxthon`
      )
      let paymail = ''
      if (this.alias) {
        paymail = this.paymail
